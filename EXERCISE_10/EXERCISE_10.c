...
int hour = 15, minute = 26, second = 55;
...
void shiftRightAnimation(void);
/* USER CODE END PFP */

/* USER CODE BEGIN 0 */
void setTimer7seg(int duration) { timer7seg = duration; }
void setTimerClock(int duration) { timerClock = duration; }
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim) {
  if (htim->Instance == TIM2) {
    ...
    if(led_matrix_index >= MAX_LED_MATRIX) led_matrix_index = 0;
  }
}
/* USER CODE END 0 */
